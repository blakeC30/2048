...
public class BoardComponent extends JComponent {
    ...
    public void paintBoard(Graphics g) {
        int cell_size_x =
                (BOARD_COMPONENT_SIZE - (SPACE_BETWEEN_CELLS * (num_cols - 1))) / num_cols;
        int cell_size_y =
                (BOARD_COMPONENT_SIZE - (SPACE_BETWEEN_CELLS * (num_rows - 1))) / num_rows;
        ...
        for (int r = 0; r < num_rows; r++) {
            for (int c = 0; c < num_cols; c++) {
                ...
                if (value != EMPTY_CELL_VALUE) {
                    if (value == TWO || value == FOUR) g.setColor(new Color(119, 109, 101));
                    else g.setColor(new Color(253, 243, 240));
                    int value_x =
                            x_coordinate + (cell_size_x / 2) - (String.valueOf(value).length() * 9);
